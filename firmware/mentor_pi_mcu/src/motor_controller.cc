...
// These corrected incremental-PID starting values preserve the scale of the
// legacy profiles while fixing its unused proportional-difference term. They
// are safe-bounded but remain release-provisional until D3 motor HIL records
// the final gains, filter, and deadband for each physical motor profile.
...
MotorController::MotorController(MotorControlConfiguration configuration)
  ...
  if (!configuration_.closed_loop_enabled || !configuration_valid) {
    configuration_.closed_loop_enabled = false;
    configuration_.maximum_accepted_rps = 0.0F;
    configuration_.output_limit_permille = 0;
  }
}
...
Result MotorController::AcceptCommand(const MotorCommand& command,
  ...
  for (std::size_t index = 0; index < kMotorCount; ++index) {
    ...
    if (!configuration_.closed_loop_enabled) {
      RecordRejectedCommand(command.update_mask);
...
std::array<std::int16_t, kMotorCount> MotorController::ControlStep(
  ...
  for (std::size_t index = 0; index < kMotorCount; ++index) {
    ...
    const float filter_weight = profile_->pid.velocity_filter_new_weight;
    ...
    const float proportional =
        profile_->pid.proportional_gain * (error - pid.previous_error);
    const float integral = profile_->pid.integral_gain * period_seconds * error;
    const float derivative =
        profile_->pid.derivative_gain *
        (error - 2.0F * pid.previous_error + pid.error_before_previous) /
        period_seconds;

    float candidate =
        pid.accumulated_output + proportional + integral + derivative;
    // Conditional integration prevents the integral term from driving farther
    // into saturation while retaining proportional/derivative recovery.
    ...
    if ((candidate > output_limit && error > 0.0F) ||
      ...
      candidate = pid.accumulated_output + proportional + derivative;
    }
    pid.accumulated_output = ClampOutput(candidate, output_limit);
    pid.error_before_previous = pid.previous_error;
    pid.previous_error = error;

    const float absolute_output = std::fabs(pid.accumulated_output);
    ...
    if (absolute_output > 0.0F &&
      ...
      channel.output_permille =
          pid.accumulated_output > 0.0F
              ? effective_deadband
              : static_cast<std::int16_t>(-effective_deadband);
    } else {
      channel.output_permille =
          static_cast<std::int16_t>(std::lround(pid.accumulated_output));
    }
