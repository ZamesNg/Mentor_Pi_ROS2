...
| ID | Requirement | Verification |
|---|---|---|
| PLAT-001 | The deployment host **shall** run ROS 2 Jazzy on Ubuntu 24.04 amd64 or arm64. Apple Silicon development is supported only through an Ubuntu 24.04 arm64 virtual machine with USB passthrough; native macOS is not a deployment target. | Test |
| PLAT-002 | The MCU software **shall** target STM32F407VET6 using STM32 HAL, FreeRTOS, and the ROS 2 Jazzy-compatible micro-ROS client stack. | Inspection |
...
| PLAT-004 | The firmware **shall** compile with warnings treated as errors for project-owned sources. | Test |
...
| TRN-004 | After transport recovery, the MCU **shall** recreate its ROS entities and resume publication without requiring a power cycle. DC motors shall remain stopped until a new valid zero/stop command arrives or, in an explicitly acknowledged commissioning image, a new valid bounded motion command arrives. | Test |
...
| RT-004 | At 500 motor commands per second, the MCU **shall** remain responsive, continue its 100 Hz motor loop and required publications, honor the 200 ms lease, and recover without reset. A normal motor-locked image uses zero targets for this software/transport test; powered alternating targets are permitted only in the guarded commissioning/HIL procedure from SAFE-006 and SAFE-007. Intermediate best-effort commands may be coalesced. | Test |
