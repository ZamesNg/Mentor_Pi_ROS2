...
| HOST-005 | The optional onboard plugin-based tracker **shall** implement the scheduling, validation, vehicle model, deadline, ADRC/MPC selection, fallback, and zero-output behavior in [tracking-controller.md](tracking-controller.md), and shall default to disabled. It shall not perform high-level planning or arbitrary frame transforms; its sole onboard reference-point conversion is the configured Ackermann rear-axle-to-geometry-center offset (`VER-TRACK-UNIT-001`, `VER-TRACK-INT-001`, `VER-TRACK-RDK-001`). | Inspection, Test |
...
| SAFE-007 | Before the first powered motor command on a board, checkout **shall** complete a passive, manually driven encoder direction test with motor outputs disabled. Wheels shall then remain raised or equivalently guarded and the board shall use a current-limited supply for every unqualified powered test. Every encoder factor is `+1`; JGA27's separate drive-output factor is provisionally `-1` from legacy evidence. Neither the drive polarity nor any motor ADRC profile is release-qualified until the required motor HIL passes and its evidence is recorded. | Review, Test |
