...
| ID | Requirement | Verification |
|---|---|---|
| PLAT-001 | The production onboard computer **shall** be an arm64 RDK X5 using Docker Engine on Ubuntu 22.04. Firmware, source-locked micro-ROS generation, the compiled Agent, host packages, lightweight tests, interactive development, handoff, and production runtime shall use one pinned, content-identified, architecture-native Ubuntu 22.04/Humble project image on each computer. The RDK shall pull only its project-image base. Normal computers shall additionally use one pinned Ubuntu 24.04/Clang 18 quality image for formatting, tidy, coverage, and fuzzing. QEMU and cross-architecture emulation are prohibited. Existing host ROS shall be left untouched and never sourced. Only udev configuration and STM32CubeProgrammer flashing shall run on the host. The shared build-job limit shall be `max(1, min(CPU count, 4, floor(MemAvailable / 2 GiB)))`, with an explicit positive override no greater than CPU count. The runtime image and `make shell` shall provide pinned Oh My Zsh, completion, autosuggestions, and syntax highlighting, while builds, CI, Make recipes, automation, and the non-interactive runtime entrypoint remain Bash. Native macOS is not a deployment target. | Test |
