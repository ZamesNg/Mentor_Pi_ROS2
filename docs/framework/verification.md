...
## Component-layout and command-boundary gates

`VER-IMG-UNIFIED-001` is retired by ADR-0003 and reserved for historical
reports. `VER-COMPONENT-LAYOUT-001` verifies the three independent component
graphs, canonical interface/SDK compatibility, exact tutorial paths, and the
absence of legacy workspace, production Docker, OCI, QEMU, and container
handoff paths. It also verifies that `.devcontainer/` is development-only.

`VER-TRACK-UNIT-001`, `VER-TRACK-INT-001`, and `VER-TRACK-RDK-001` are retired.
Outer trajectory tracking and global pose are application-owned. The retained
host gate verifies a 50 Hz controller loop, latest stamped-reference input,
local receipt-time freshness for zero, stale, and current sender stamps, and the
100 ms controller timeout.
...
|---|---|
| Ubuntu 22.04 native | Firmware and Agent builds, ROS application build/test/runtime, service installation, deterministic fake clock/HAL, sanitizers, unit tests, and fuzzers on amd64/arm64. |
| VS Code Dev Container | Development build/test for macOS and non-Ubuntu-22.04 Linux; never onboard, HIL, systemd, flashing, or release evidence. |
...
Before any powered motor case, archive and verify the exact default ADRC build,
then manually rotate each raised wheel with drive outputs disabled and record
raw MCU and ROS encoder direction. A powered pre-qualification case shall
...
firmware limits reduce checkout risk; none is evidence that any ADRC profile,
...
| `VER-BUILD-HOST-001` | On native Ubuntu 22.04/Humble amd64 and arm64, rosdep/vcs/colcon list, build, and test the three project packages and three pinned controller-overlay packages with zero first-party warnings. The same component commands smoke in the VS Code Dev Container as development evidence. macOS and other Linux native execution rejects build/test with a Dev Container instruction. Project-owned production nodes and the data path contain no Python node, `rclpy` use, or serial translation bridge; Python ROS launch remains permitted orchestration. |
| `VER-IMG-UNIFIED-001` | Retired by ADR-0003; historical results have no acceptance value for the native component topology. |
| `VER-COMPONENT-LAYOUT-001` | Inspection and path-policy tests prove one Git history, no submodules, independent `firmware`, `micro_ros_agent`, and `ros2_ws` Make/build trees, canonical interfaces plus hash-bound checked firmware SDK, exactly three project packages and three pinned controller-overlay packages, exactly two ordered host/onboard tutorial tracks, and `.devcontainer/` as the sole Docker definition. The old workspace, production Docker/runtime, devcontainer alternatives, OCI, QEMU, container entrypoints/handoffs, `make start`, and Agent ownership in ROS launch are absent. |
| `VER-TRACK-UNIT-001` | Retired; outer trajectory tracking is application-owned. |
| `VER-TRACK-INT-001` | Retired; outer trajectory tracking is application-owned. |
| `VER-TRACK-RDK-001` | Retired; outer trajectory tracking is application-owned. |
| `VER-POSE-MAP-001` | Launch-contract tests require controller `enable_odom_tf=false`, keep controller odometry on private remapped topics, expose no project-owned public global pose or TF adapter, and preserve the Ackermann `base_footprint -> rear_axle_footprint` offset, wheel links, and `imu_link`. External applications exclusively own global pose and `map -> <robot>/base_footprint`. |
| `VER-SIM-001` | Plugin unit tests cover exact interfaces, lifecycle transitions, finite saturation, wheel acceleration ramps and reversals, trapezoidal position integration, Ackermann steering averaging/rate/angle limits, invalid inputs, and deactivation. Headless launch tests start both vehicle types, require `vehicle` active, verify a runtime controller-manager update rate of at least 50 Hz, drive one direction with zero-stamped references at exactly 50 Hz, replace it with deliberately stale nonzero-stamped references at 60 Hz, then alternate references above 50 Hz and prove the newest wins. The same test requires every sender-stamp form to refresh local receipt-time freshness without warnings or stale-stamp rejection and observes the zero controller command 100--120 ms after the final locally sent reference, with only a small same-host publish/receipt allowance. Graph tests reject Agent, supervisor, firmware, motor-state, heartbeat, authorization, tracker, global-pose adapter, and public odometry endpoints. Launch-contract tests bind both profiles to exactly 50 Hz and 100 ms, verify simulation-only URDF plugins and private controller odometry remaps, and require a separate Foxglove bridge and absence of `/clock`. |
...
| `VER-BUILD-MOTOR-GATE-001` | For SAFE-006, firmware `build`, `verify`, `package`, `flash`, and `flash-package` select the single `NORMAL_CLOSED_LOOP_DEFAULT` artifact with `control_mode=CLOSED_LOOP`. Tests reject alternate classification/mode, stale metadata, changed source/dependency/artifact hashes, wrong robot namespace, a transferred manifest that differs from its separately recorded digest, a non-ARM ELF, programmer failure, and missing read-back success. Packaging contains exactly one ELF/Hex/Bin/Map set plus build metadata/mode. Software evidence does not qualify ADRC performance or powered motion. |
...
| `VER-API-001` | The MCU node is exactly `/mentor_pi/controller` and owns exactly the seven publishers, seven subscriptions, and seven services in the ROS contract. `/rosout`, parameter, parameter-event, statistics, action, and `/clock` endpoints are absent. Names, generated types, fields, units, and direction match exactly; no legacy alias exists. Other expected processes, such as the Agent and C++ supervisor, may have their own nodes. On native Ubuntu, one 60-second zero-command capture records motor, IMU, PWM, heartbeat, battery, and diagnostics together; `ROS_SUPER_CLIENT=TRUE`, when required for discovery, is scoped to the recorder process. Rates are 50/50/20/2/1/1 Hz within ±5%, motor and IMU source gaps are at most 25 ms with no periodic 30 ms/10 ms pattern, heartbeat sequences are contiguous, and session/reconnect/error counters do not change. |
| `VER-SERIALIZATION-001` | Host and firmware generate types from the same IDL. Introspection proves every endpoint type has a finite maximum serialized size, every array is fixed, every string is bounded, and no unbounded sequence/string exists. CDR round trips cover zero/default, every boundary, and maximum-size values for every message, request, and response with byte-for-byte field equality and no handwritten wire struct. The pinned Humble generated type support is normative and shall serialize `SetMotorAdrc_Request` to exactly 216 bytes and report a 388-byte maximum message-field payload for `ControllerDiagnostics`; serialization of the latter through the pinned RMW/CDR path shall produce exactly 392 bytes including the four-byte encapsulation. Generated CDR plus XRCE headers for every maximum sample is no greater than the 512-byte MTU and requires no endpoint-level fragmentation; conflicting handwritten size arithmetic fails the test. |
| `VER-CONFIG-001` | The non-root `mentor-pi-agent.service` is the sole `/dev/mentor_pi_mcu` owner, starts at boot, restarts after serial loss, uses a versioned verified Agent, and never starts ROS applications. Manual `controller.launch.py` starts only the supervisor and shuts its containing application launch down on supervisor exit. The supervisor accepts every valid nonlinear ADRC/filter/directional-floor YAML boundary, including the bandwidth and `disturbance_limit <= b0 * 1000` cross-constraints, and rejects unknown keys, wrong types/counts, invalid models, non-finite values, and adjacent out-of-range values while keeping authorization zero. Exact request-content, response-profile, and generation/session retry tests reject inconsistent, late, stale, failed, or exhausted configuration, reapply every array in order after new sessions, and replay no actuator or bus state. |
...
| `VER-UNIT-MOTOR-GATE-001` | For SAFE-006 and SAFE-007, the default closed-loop motor configuration accepts every valid selected zero or bounded nonzero subset, ignores unselected parameter garbage, rejects a selected non-finite or out-of-range nonlinear ADRC/filter/floor value atomically without applying output or refreshing any lease, and has no alternate mode branch. Closed-loop tests prove dimension-preserving `fal` boundaries, nonlinear ESO/controller evolution with actual period, filtered-speed input, known decay and disturbance leakage, bounded disturbance compensation, controller/observer exponents and thresholds, independent positive/negative target floors no greater than 250 permille, opposing-output floor bypass, positive and negative +/-1000-permille saturation, timing failure, target-sign-reversal reset, all four legacy model limits, the 6 RPS implementation ceiling, independent leases, state resets, and session-loss disarming. `set_adrc` tests cover every result code, boundary and cross-field constraint, atomic multi-channel application, state reset, reconnect retention, reset/model-change volatility, and failure masks. Portable tests assert raw direction preservation, the uniform `-1` physical bridge polarity on M1–M4 for every model, semantic observer input before that inversion, connector ordering, and the bidirectional host ROS↔MCU sign map. |
| `VER-UNIT-CHASSIS-ADRC-001` | For HOST-006, common tests cover first-order measurement-LPF period response, initialization/reset and invalid configuration plus ADRC initialization, observer evolution, signed input gain, reset, and invalid timing. Hardware-plugin tests cover filter-parameter validation, wheel/connector mapping, mecanum body-axis correction, Ackermann rear differential preservation and low-speed steering behavior, gyro-Z input, output/profile bounds, and zero/error behavior for missing, invalid, or stale IMU feedback. Launch-contract tests bind the checked YAML geometry, exactly 50 Hz update rate, 100 ms reference timeout, 5 Hz measurement filters, and ADRC defaults into both URDF hardware plugins. |
| `VER-UNIT-MBOX-001` | Motor, PWM, LED, and OLED tests first publish a complete state and then update one selected element in the same session; every selected element changes and every unselected element remains unchanged. RGB tests replace only host-owned RGB2 and prove masks selecting RGB1 are rejected without changing either pixel. Multiple writes to the same selected element before one read expose only the newest generation and increment overwrite accounting exactly as the ROS contract specifies. For each multi-field merged mailbox, leave a selected field unread, reset its session ownership, then publish a disjoint field: the old field-valid generation is zero, its pending value is absent, the new field is present, and the monotonic mailbox generation does not restart. The triple-buffer test races producer `Publish` plus producer `DiscardLatest` against `ConsumeLatest`; every consumed object remains whole and strictly newer, a discard winning after the consumer's initial unread observation returns no value, and the final non-discarded generation remains observable. Controller tests repeat the reset at a real session transition: unread gen-1 motor/PWM/LED/RGB2/OLED fields never arm, target, or render in gen 2; stale whole-command bus/buzzer work performs no hardware transaction. A separate contrast proves applied LED/OLED state and RGB2 hold across reconnect while RGB1 remains exclusively system-owned. Bus motion and buzzer complete-state slots otherwise expose only the newest same-session complete write. All storage remains at its documented depth. |
...
| `VER-UNIT-SVC-001` | Each non-bus slot admits one request and returns `BUSY` to a concurrent request; local worker completion or typed failure occurs within 50 ms. The completion and request-group cursors give each continuously occupied/ready local or shared slot one turn per five service-class opportunities, including the ADRC slot. The shared bus slot is non-preemptible: while occupied, every additional bus request, including stop, returns `BUSY`, and the three busy endpoints rotate without starvation. When free with simultaneous work, stop dispatches first and get/configure alternate deterministically; every accepted service dispatches before pending motion, and only the newest pending move remains. A stop arriving during a move completes the current UART frame and interrupts before the next move frame; it never cuts an in-flight frame or preempts an accepted get/configure service. Every unsent frame from the interrupted move batch and every pending pre-stop move generation is discarded. The explicit sequence A active → B pending → stop transmits neither A's remainder nor B; only a move accepted after stop may restart traffic. Bus work finishes or returns `TIMEOUT` by 200 ms MCU time and the host returns by 250 ms. Generation-tagged late completions cannot satisfy a later request. |
| `VER-UNIT-MW-001` | Instrument every `MicroRosTask` middleware boundary: `rclc_executor_spin_some`, nonblocking `rcl_take_request`, best-effort and reliable publish, `rcl_send_response`, WAIT_AGENT ping, ACTIVE time sync, and each incremental create/finalize step. The ACTIVE spin timeout is exactly zero and the notification-aware outer wait is at most 1 ms. With the fault proxy withholding input, replies, or reliable ACKs at each boundary, transport-read slices, ACTIVE time sync, reliable publish/response, and remote finalizer calls are at most 10 ms each; WAIT_AGENT ping and creation-time sync are at most 20 ms; each entity-creation call is at most 40 ms and the complete creation phase at most 2 s; complete remote destruction is at most 500 ms; and each custom write obeys its length-derived deadline. No individual/incremental step creates a `MicroRosTask` heartbeat interval over 100 ms, waits for a peripheral, grows storage, or evades the documented error/teardown path. Portable wraparound tests prove strict service → reliable-telemetry → maintenance rotation, a single shared blocking-operation permit across response/reliable-publication/time-sync work, no ACTIVE ping, retained WAIT_AGENT probing, and ACTIVE-entry due phases of 0 ms heartbeat, 250 ms battery, and 750 ms diagnostics. The permit is denied whenever best-effort telemetry is overdue or fewer than 12 ms remain before the earliest motor/PWM/IMU deadline. Injecting a 10 ms operation at every eligible phase preserves best-effort deadlines, produces no periodic 30 ms/10 ms motor or IMU pattern, and leaves deferred blocking work pending for a later eligible turn. At most one due best-effort publication and, on its selected class, one due reliable publication starts per slice; both publisher sets rotate fairly. A deliberate second permit request tears down the test session. |
...
|---|---|---|
| `VER-HIL-LED-001` | `ROS-LED-01`, `HW-LED-01` | LED2 and LED3 independently perform steady off/on, one finite cycle, multiple finite cycles, and indefinite repetition followed by replacement. Measured on/off intervals are within ±2% or 2 ms, whichever is larger. A ROS command for LED1 is rejected without changing it; LED1 continues its 500 ms off/500 ms on system heartbeat independently of ROS. |
...
| `VER-HIL-MOT-001` | `ROS-MOT-01`, `HW-MOT-01` | The test is staged with the exact verified default ADRC artifact. First, keep all bridge outputs at zero, manually rotate each raised wheel in the declared positive direction, and record raw MCU `motors/state` and converted ROS joint signs. Second, use raised-wheel/equivalent guarding and a current-limited supply; exercise one channel and one sign at a time with deliberately bounded targets, require encoder and operator-observed physical direction to agree, verify output limiting and stop/lease/session kill paths, and stop on wrong sign, unexpected current, oscillation, or cross-channel response. Recorded HIL must qualify or replace the ROS↔MCU chassis sign, each channel/model encoder constant, ADRC/filter/minimum-drive floor profile, full documented RPS range, subset merge, zero behavior, model-change behavior, and independent state before powered-motion or ADRC-performance claims are made. |
...
| `VER-HIL-RGB-001` | `HW-RGB-01` | RGB2 mask 2 produces exact off, primary, secondary, and white component values and a conforming serialized waveform. Zero, masks 1/3, and unknown masks are rejected without changing either pixel. On RGB1, red toggles after each successful micro-ROS heartbeat publication, green pulses on sampled TX progress, blue pulses on sampled RX progress, simultaneous activity composes all active components, and a closed transport clears green/blue without changing the last red heartbeat state. The status path performs no allocation or UART-ISR work. |
...
| `VER-HIL-IMU-001` | `ROS-IMU-01`, `HW-IMU-01` | Before the transform is enabled, a successful raw QMI8658 acquisition precedes the single expected transform-`UNSUPPORTED` diagnostic; an injected raw-I2C error increments the IMU error/timeout diagnostics and cannot be reported as the expected characterization condition. Register readback proves accelerometer ±4 g and gyroscope ±128 degrees/s at 250 Hz with CTRL5 `0x11`, enabling both mode-00 LPFs at 2.66% of ODR (approximately 6.65 Hz). Six-face gravity and positive rotation about each board axis establish and record the QMI8658-to-`imu_link` signed permutation before the endpoint is enabled. At rest, the vector magnitude of `linear_acceleration_m_s2` is 9.80665 m/s² ±5% and all three `angular_velocity_rad_s` components are within the QMI8658 documented zero-rate tolerance. Axes/signs match the fixed contractual frame, conversion is exactly g×9.80665 and degrees/s×π/180, range endpoints are bounded by ±39.2266 m/s² and ±2.234021 rad/s, `stamp` is monotonic after sync, `valid` is true for successful samples and false after an injected read failure, no orientation/covariance/string field exists, and rate is 50 Hz ±5%. |
...
LED commands at 1 Hz, one finite buzzer pattern every 10 seconds, RGB2 commands
...
default ADRC image on the guarded/current-limited fixture. A software-only
...
pre-qualification variant does not qualify ADRC or polarity for release.
...
2. For every current-session `MotorControlTask` mailbox consumption that
   ...
   ADRC/output update remains 100 Hz.
...
pattern per minute, RGB2 at 30 Hz, OLED at 1 Hz, and all nominal
...
exact default ADRC release candidate on a guarded/current-limited fixture;
...
| `VER-FAULT-TX-001` | Keep USART1 DMA and UART TC completion normal and run independent Agent fault phases. In separate recovered sessions, withhold the ACK for one non-heartbeat reliable publication, one service response, and the 2 Hz heartbeat. Each operation returns/fails within its 10 ms XRCE session timeout, reliable history stays within its fixed eight-by-512-byte capacity, and fatal session teardown occurs immediately without an ACTIVE ping. Independent XRCE capture records the attempted operation, withheld ACK, bounded history use, and teardown; source inspection and instrumentation prove no Agent ping is attempted while ACTIVE. In every phase `transport_tx_timeouts` is unchanged because physical writes complete, motors are safe, no stale generation is emitted, and all endpoints recover within 5 s after the Agent resumes. |
...
| `PLAT-005` | `VER-BUILD-FW-001`, `VER-COMPONENT-LAYOUT-001` |
| `PLAT-006` | `VER-COMPONENT-LAYOUT-001` |
...
| `HOST-006` | `VER-UNIT-CHASSIS-ADRC-001` |
| `HOST-007` | `VER-POSE-MAP-001` |
| `HOST-008` | `VER-SIM-001` |
...
[onboard Tutorial 08](../tutorials/onboard/08-evidence-and-qualification.md) as the
