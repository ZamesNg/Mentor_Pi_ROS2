...
| ID | Requirement | Verification |
|---|---|---|
| PLAT-001 | The production onboard computer **shall** be an arm64 RDK X5 using Docker Engine on Ubuntu 22.04. Firmware, source-locked micro-ROS generation, the compiled Agent, host packages, lightweight tests, interactive development, handoff, and production runtime shall use one pinned, content-identified Ubuntu 22.04/Humble project image for each target architecture. The production image and every artifact installed on the RDK shall target arm64. The RDK shall pull only its project-image base. Normal computers shall additionally use one pinned Ubuntu 24.04/Clang 18 quality image for formatting, tidy, coverage, and fuzzing. Existing host ROS shall be left untouched and never sourced. Only udev configuration and STM32CubeProgrammer flashing shall run on the host. Ordinary native commands shall use `max(1, min(CPU count, 4, floor(MemAvailable / 2 GiB)))`, with an explicit positive override no greater than CPU count. The dedicated QEMU handoff exception is specified by PLAT-006. The runtime image and `make shell` shall provide pinned Oh My Zsh, completion, autosuggestions, and syntax highlighting, while builds, CI, Make recipes, automation, and the non-interactive runtime entrypoint remain Bash. Native macOS is not a deployment target. | Test |
...
| PLAT-005 | The unified Humble image **shall** use the signed 2026-08-07 snapshot and an exact architecture-specific package lock, install and checksum Arm GNU 13.2.1 exactly once, and fail for an unknown architecture or installed-version mismatch. Its identity shall cover the base digest, Dockerfile, ROS lock, zsh configuration, and dependency locks (`VER-IMG-UNIFIED-001`). | Test |
| PLAT-006 | The standard RDK artifact workflow on an amd64 development computer **shall** be `make rdk-handoff`, which shall build and export the arm64 project image and complete arm64 host handoff using QEMU/binfmt emulation without requiring a second authorization flag. `make host` and other ordinary development commands shall remain native to the current computer; the dedicated `rdk-handoff` target is the explicit selection of the cross-architecture workflow. It shall select only arm64 base images, package locks, tools, binaries, and OCI platform metadata; shall fail rather than fall back to amd64 when QEMU/binfmt is unavailable or any host/target mismatch occurs; and shall record `build_execution=qemu-emulated`, `build_host_architecture=amd64`, `target_architecture=arm64`, and `native_target_validated=0` in the handoff evidence. The handoff shall default to `min(8, online CPU count)` package workers, constrain per-package CMake/Ninja work to one worker, print the selection, and honor an explicit positive `RRCLITE_BUILD_JOBS` no greater than the CPU count. It shall atomically checkpoint per-stage input identities, completed stages, and output checksums; automatically resume the active release and its incremental host build; refresh a changed private source context while preserving stages whose own inputs and verified output remain unchanged; invalidate and rebuild only affected/dependent stages; preserve failed work and the original UTC release ID; reject malformed, symbolic, or tampered state; permit an explicit full fresh reset; run runtime smoke against packaged artifacts without another host build; and clear active disposable work only after the final checksummed bundle succeeds. Native compilation on the RDK is optional diagnostic evidence and is not required for release. The RDK shall load and install the handoff without rebuilding the host workspace. The onboard operator path shall use compact `make rdk-receive`, `make flash-production`, and `make production-install` actions rather than timestamp placeholders or manual archive, image, Agent, host-prefix, and systemd installation sequences; those actions shall preserve the same platform, checksum, symlink, image-ID, executable-hash, identity, inactive-target, and unit-verification gates. Emulated compilation and packaging shall not satisfy native RDK image-load, runtime, memory, tracker-deadline, peripheral, HIL, or release-qualification evidence. | Test |
...
| ID | Requirement | Verification |
|---|---|---|
| HOST-001 | A project-owned C++17 `rclcpp` node named `/mentor_pi/configuration_supervisor` **shall** validate the exact YAML schema in [architecture.md](architecture.md) for motor model, four PWM-servo offsets, and battery threshold. Invalid configuration shall keep project-owned host motion disabled. | Inspection, Test |
| HOST-002 | The supervisor **shall** treat first discovery, graph/heartbeat reappearance, an `agent_session_id` change, or a wrap-aware MCU uptime discontinuity as a new session and shall apply motor model, PWM offsets, then battery threshold in that order. Project-owned host motion shall remain disabled until all three return `OK`. | Test |
...
| HOST-005 | The optional onboard tracker **shall** implement the scheduling, validation, model, deadline, fallback, and zero-output behavior in [tracking-controller.md](tracking-controller.md), and shall default to disabled. It shall not perform high-level planning or frame transforms (`VER-TRACK-UNIT-001`, `VER-TRACK-INT-001`, `VER-TRACK-RDK-001`). | Inspection, Test |
...
| TRN-002 | The host **shall** run a micro-ROS Agent configured for a serial device selected by a launch/configuration argument, whose default is `/dev/mentor_pi_mcu`. | Inspection, Test |
...
| CTRL-004 | A motor-profile change **shall** be rejected while any motor has a nonzero target; a successful change shall reset all motor PID state. | Test |
...
| SAFE-006 | `make firmware`, `make flash`, and `make start` **shall** build, verify, flash, and run only the `NORMAL_CLOSED_LOOP_DEFAULT` PID artifact with `control_mode=CLOSED_LOOP`; no alternate motor-authority mode or runtime unlock is supported. The controller shall enforce the active model's RPS limit and the 6 RPS implementation ceiling, clamp output to +/-1000 permille, reject an invalid motor command atomically without refreshing any selected lease, and require the session-bound host configuration gate before project-owned host motion starts. | Inspection, Test |
