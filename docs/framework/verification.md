...
| Agent loopback | ROS 2 Jazzy and the micro-ROS Agent connected through a pseudo-terminal or serial fault proxy; no physical actuators. |
...
| `VER-BUILD-HOST-001` | On supported Ubuntu 24.04 with ROS 2 Jazzy, debug, release, ASan/UBSan, and TSan jobs build and test all first-party host C++ and generated interfaces with zero first-party warnings. Project-owned production nodes and the control/data path contain no Python node, script, `rclpy` use, or serial translation bridge. Pinned upstream ROS tools and their Python implementation/dependencies are permitted and reported separately. |
| `VER-BUILD-FW-001` | Two clean builds with the pinned Arm toolchain produce identical loadable sections. The target is STM32F407VET6 with the pinned HAL, FreeRTOS, and Jazzy-compatible micro-ROS client; every generated-library repository is detached at the checked-in source lock (including the temporary `geometry2/tf2_msgs` source), missing/unexpected repositories fail the build, the archive matches its reviewed SHA-256, first-party warnings are errors, and ELF, BIN/HEX, map, and size reports are emitted. |
| `VER-BUILD-MOTOR-GATE-001` | For SAFE-006, the supported build with commissioning variables absent or `RRCLITE_MOTOR_COMMISSIONING=0` reports `LOCKED` and compiles closed-loop authority out. `RRCLITE_MOTOR_COMMISSIONING=1` without the exact `RRCLITE_MOTOR_COMMISSIONING_ACK=MOTORS_RAISED` value fails before producing artifacts; direct CMake configuration with commissioning on and a missing/wrong acknowledgement also fails. Supplying both exact values reports `COMMISSIONING` and produces an image whose compiled limits are exactly 0.25 RPS and 300 permille. Commissioning artifacts are distinctly labelled/hashed in the test record and are never substituted for a production release image. |
