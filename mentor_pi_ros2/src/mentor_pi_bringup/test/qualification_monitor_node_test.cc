...
void TestDefaultIsStrictAndImmutable() {
  ...
  Expect(!monitor.node->get_parameter("allow_missing_oled").as_bool(),
         "missing-OLED exception defaults false");
...
void TestExplicitCharacterizationOverride() {
  ...
  options.parameter_overrides(
      {rclcpp::Parameter("imu_characterization_mode", true),
       rclcpp::Parameter("allow_missing_oled", true)});
  ...
  Expect(monitor.node->get_parameter("allow_missing_oled").as_bool(),
         "explicit missing-OLED override is applied");
