...
| ID | Requirement | Verification |
|---|---|---|
| PLAT-001 | The production onboard computer **shall** be an arm64 RDK X5 using Docker Engine on Ubuntu 22.04. Source-locked micro-ROS generation, the compiled Agent, host packages, interactive development, and production runtime shall use pinned, content-identified, architecture-native Ubuntu 22.04/Humble images on both computer types. The non-ROS cross-compiler and quality images shall use pinned architecture-native Ubuntu 24.04 toolchain definitions; their outputs still target the STM32 or Humble-compatible source tree and are not production host runtimes. QEMU and cross-architecture emulation are prohibited. Existing host ROS shall be left untouched and never sourced. Only udev configuration and STM32CubeProgrammer flashing shall run on the host. The shared build-job limit shall be `max(1, min(CPU count, 4, floor(MemAvailable / 2 GiB)))`, with an explicit positive override no greater than CPU count. The RDK setup and release gate shall omit fuzzing, coverage, and the full Clang analysis suite. The runtime image and `make shell` shall provide pinned Oh My Zsh, completion, autosuggestions, and syntax highlighting, while builds, CI, Make recipes, automation, and the non-interactive runtime entrypoint remain Bash. Native macOS is not a deployment target. | Test |
