...
| --- | --- | --- | --- |
| `firmware/mentor_pi_mcu/build/stm32/` | `LOCKED` | `2bd7fa3e0da06d293b9d72cadcad7ad4fc2bc5735cd42fc4ad99573710d99864` | Historical provenance verified with the measured wheel/IMU profile; rebuild and hardware flash are pending for the firmware-owned LED1/RGB1 status change. |
...
| BOARD-PROFILE-001 | Connector mapping verified; raw direction is fixture-specific | Passive one-wheel-at-a-time capture established `front-left=M1`, `front-right=M3`, `rear-left=M2`, and `rear-right=M4`. The 2026-08-07 fixture recorded forward deltas M1 `+742`, M2 `+1915`, M3 `-1370`, M4 `-1612`; the 2026-08-17 power-disconnected `ackermann_0` capture instead recorded M2 `-1054`, M4 `+3415`. | Connector ownership is stable, but the contradictory raw directions cannot qualify a fleet-wide electrical sign. The older powered M1 result belongs only to its fixture. | The maintained sign contract is explicit in `docs/framework/verified-hardware-profile.md`; firmware retains raw MCU direction and the host owns the only ROS chassis sign map. | Repeat passive one-wheel-at-a-time verification on every vehicle before powered use; do not transfer raw signs between vehicles. |
| MOTOR-SIGN-COORDINATE-001 | Superseded by `MOTOR-CONTROLLER-POLARITY-002` | Commit `9b087ea` introduced separate encoder/output model signs while the host map remained unchanged. On `ackermann_0`, forward manual rear-wheel rotation then appeared negative on both ROS joints. | Multiple sign layers split target, measurement, and output across inconsistent coordinates. The first correction retained raw encoder state and removed the extra model-specific transforms, but its direct bridge-output assumption had not yet been tested under power. | MCU encoder state remains raw and the single host ROS↔MCU map remains `{-1,+1,-1,+1}` in FL,FR,RL,RR order. The later powered result establishes the bridge-boundary correction separately. | Preserve this entry as the history of the coordinate simplification; use the later polarity entry for current behavior. |
| MOTOR-CONTROLLER-POLARITY-002 | Corrected in source; guarded hardware verification pending | On `ackermann_1`, a direct M2 target of `+0.05 RPS` produced approximately `-6.93 RPS` measured feedback and a decreasing encoder count. The controller therefore increased its output instead of closing the error. The separately observed M4 zero count was confirmed by the operator to be a failed motor and was resolved by replacing that hardware. | The RRCLite bridge command and raw encoder response have negative plant polarity. Sending the semantic LADRC output directly to the bridge turns velocity feedback positive. This failure is inside the MCU loop and cannot be corrected by changing only the ROS wheel sign. | Targets, raw measurements, LADRC state, and observer input remain in one semantic MCU coordinate. Physical bridge duty is the negative semantic LADRC output for M1–M4 and all four motor models. No model-specific or channel-specific sign exists. The host map remains `{-1,+1,-1,+1}`. | Focused domain/controller tests cover both command signs, all channels, all models, saturation, and observer behavior. Flash `ackermann_1` first and perform the guarded `±0.01 RPS` one-channel test before fleet rollout. |
| ACKERMANN-JOINT-ORDER-001 | Corrected in source; deployment pending | The local controller YAML listed left then right, while Humble `steering_controllers_library` emits traction and steering command arrays right then left. | Straight commands hid the ordering error; turns assigned calculated inner/outer wheel speeds to the opposite joints. | Both Ackermann lists now use right then left, matching the upstream controller contract, with a launch-contract regression. | Verify controller activation and inspect a bounded zero-power turning reference before powered HIL. |
...
| RGB-STATUS-001 | Corrected; hardware observation pending | Earlier status ownership conflicted with the requested board indication. | A host-commanded status pixel can overwrite or imitate firmware state. | LED1 is firmware-owned and blinks at 1 Hz as the system heartbeat; LED2/LED3 are host-controlled. RGB1 is firmware-owned: red toggles after successful `/mentor_pi/heartbeat` publications, green pulses for sampled TX progress, and blue pulses for sampled RX progress. Only RGB2 remains host-controlled. | Unit and controller integration tests cover heartbeat gating, sampling, pulse expiry, combined colors, transport closure, and timer wrap. Observe both indicators after flashing; this is not yet hardware verified. |
