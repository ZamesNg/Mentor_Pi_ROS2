...
| `VER-UNIT-CHASSIS-ADRC-001` | For HOST-006, common tests cover first-order ADRC initialization, observer evolution, signed input gain, reset, and invalid timing. Hardware-plugin tests cover wheel/connector mapping, mecanum body-axis correction, Ackermann rear differential preservation and low-speed steering centering, gyro-Z input, output/profile bounds, and zero/error behavior for missing, invalid, or stale IMU feedback. Launch-contract tests bind the checked YAML geometry, timeout, and ADRC defaults into both URDF hardware plugins. |
