...
host gate verifies stamped controller input, zero-stamp conversion to local
receipt time, and the unchanged 100 ms controller timeout.
...
| `VER-SIM-001` | Plugin unit tests cover exact interfaces, lifecycle transitions, finite saturation, wheel acceleration ramps and reversals, trapezoidal position integration, Ackermann steering averaging/rate/angle limits, invalid inputs, and deactivation. Headless launch tests start both vehicle types, require `vehicle` active, publish best-effort zero-stamped commands, verify local receipt-time conversion without repetitive warnings, and prove the unchanged 100 ms timeout stops motion. Graph tests reject Agent, supervisor, firmware, motor-state, heartbeat, authorization, tracker, global-pose adapter, and public odometry endpoints. Launch-contract tests verify simulation-only URDF plugins, private controller odometry remaps, a separate Foxglove bridge, and absence of `/clock`. |
