...
|---|---|---|---|---|---|
| ROS-LED-01 | `~/set_led` (`LedState`) → function 1, `id,on_ms,off_ms,repeat` | `packet_led_handle()` → LED pattern timer → PD9/PD10; PD11 also indicates packets | `leds/command` (`LedCommand`), LED1/LED2 only | Only IDs 1–2 are accepted although three LEDs exist; payload size is unchecked; float seconds silently narrow to 16-bit milliseconds; LED3 ownership conflicts with packet indication. v2 validates first, reserves LED3 for successful-heartbeat indication, and rejects host attempts to override it. | `VER-HIL-LED-001`, `VER-UNIT-VAL-001` |
...
| HW-LED-01 | LEDs 1 and 2 are active-low on PD9/PD10; LED3 is active-high on PD11 | **Replace** with ROS-owned LED1/LED2 and firmware-owned LED3 | LED3 is implicitly flashed for packets, conflicting with user ownership. v2 gives LED3 explicit successful-heartbeat ownership. | `VER-HIL-LED-001` |
...
| HW-RGB-01 | Two WS2812-compatible pixels through SPI1; function 11 → `packet_RGB_Ctl_handle()` | **Replace** with ROS-owned RGB1 plus firmware RX/TX status on RGB2 | No legacy ROS endpoint; pixel mask/payload are unchecked; Python function enum is inconsistent. v2 keeps the fixed two-pixel wire shape, accepts only mask 1, and rejects attempts to override RGB2. | `VER-HIL-RGB-001`, `VER-UNIT-VAL-001` |
