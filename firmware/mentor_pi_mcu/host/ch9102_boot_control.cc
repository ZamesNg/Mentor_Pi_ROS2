...
bool HasCh9102Identity(const std::filesystem::path& tty_name) {
  ...
  std::filesystem::path node = std::filesystem::canonical(
      std::filesystem::path{"/sys/class/tty"} / tty_name / "device", error);
  if (error) {
    return false;
  }
  ...
  explicit SerialModemBackend(const std::string& device) {
    ...
    if (device.empty() || device.rfind("/dev/", 0) != 0 ||
      ...
      throw std::runtime_error(
          "device is not an existing /dev character device");
    }
...
int Main(int argc, char** argv) {
  ...
  try {
    const mentor_pi::firmware::host::BootMode mode =
        mentor_pi::firmware::host::ParseBootMode(argv[4]);
    mentor_pi::firmware::host::SerialModemBackend backend(argv[2]);
    if (!mentor_pi::firmware::host::ExecuteBootSequence(mode, &backend)) {
