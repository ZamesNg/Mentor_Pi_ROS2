...
| ID | Requirement | Verification |
|---|---|---|
| PLAT-001 | The production onboard computer **shall** be an arm64 RDK X5 running ROS 2 Humble natively on Ubuntu 22.04 without Docker. It shall support conventional rosdep/colcon use, direct ros2 launch/CLI, native source-locked micro-ROS generation, and native firmware builds with the SHA-256-verified Arm GNU 13.2.1 toolchain. Its documented interactive environment shall use zsh without installing or changing the user's zsh configuration. A normal development computer may use any other supported Ubuntu release on amd64 or arm64; it shall keep ROS out of the native OS and use the pinned Ubuntu 22.04/Humble containers, with the operator tutorial covering Ubuntu 24.04. The host-runtime image and `make shell` shall provide pinned Oh My Zsh, completion, autosuggestions, and syntax highlighting, while builds, CI, Make recipes, automation, and the non-interactive runtime entrypoint remain Bash. Architecture shall be detected from the native host, not selected by the operator. Native macOS is not a deployment target. | Test |
