...
| HOST-005 | The onboard plugin-based tracker **shall** implement the scheduling, validation, vehicle model, deadline, ADRC/MPC selection, fallback, and zero-output behavior in [tracking-controller.md](tracking-controller.md). Complete vehicle launch shall default to the profile-matched ADRC plugin; an explicit launch selection may choose MPC or disable the tracker. Both algorithms and both vehicle types shall use the same node and endpoint names. Its pose state shall be `(x_center, y_center, yaw)` copied from geometry-center vehicle odometry. It shall not perform high-level planning or arbitrary frame transforms (`VER-TRACK-UNIT-001`, `VER-TRACK-INT-001`, `VER-TRACK-RDK-001`). | Inspection, Test |
...
| HOST-007 | A common hidden odometry adapter **shall** preserve `vehicle/odometry` and `vehicle/tf_odometry`, publish `odom -> base_footprint` on standard `/tf`, convert the Ackermann controller's internal rear-axle pose, twist, and covariance to the geometry center, and pass Mecanum geometry-center odometry without an offset. `rear_axle_footprint` shall remain a fixed TF child and shall not have a separate public odometry topic (`VER-ODOM-CENTER-001`). | Inspection, Test |
| HOST-008 | Development simulation **shall** provide actuator-limited Ackermann and Mecanum `ros2_control` hardware plugins using the production controller, URDF visuals, odometry adapter, namespace, and public vehicle endpoints. It shall use deterministic 30 Hz wall time without `/clock`, Agent, supervisor, firmware endpoints, tracker, noise, slip, collision, terrain, or IMU models. Initial pose arguments shall specify the public geometry-center pose (`VER-SIM-001`). | Inspection, Test |
