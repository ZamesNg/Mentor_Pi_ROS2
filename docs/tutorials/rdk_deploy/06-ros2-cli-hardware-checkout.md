...
## 1. Inspect the production graph and interfaces

Confirm the installed production runtime is active:

```sh
cd "${HOME}/Mentor_Pi"
systemctl is-active mentor-pi-controller.target mentor-pi-runtime.service
```

The systemd runtime uses the installed PID configuration, CH9102F identity,
Agent, and host release. It runs the validated `controller.launch.py` inside
the pinned arm64 production container.
...
**Warning:** Use the production controller target, not a ros2_control vehicle
launch, for this section. Verify that no other process publishes
