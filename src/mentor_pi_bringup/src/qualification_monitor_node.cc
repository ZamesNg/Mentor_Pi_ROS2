 ...
 public:
  QualificationMonitorNode(const rclcpp::NodeOptions& options,
    ...
    if (imu_characterization_mode_) {
      RCLCPP_WARN(
          get_logger(),
          "IMU characterization mode requires the exact intentional "
          "unverified-IMU condition; it is not preflight or release evidence");
    }
 ...
 private:
    ...
    battery_subscription_ = create_subscription<BatteryState>(
        ...
        [this](BatteryState::ConstSharedPtr message) {
          core_->ObserveTelemetry(
              ...
              MonotonicNowNs(), IsBatteryStateValid(*message));
        });
  ...
  void CheckCompletion() {
    ...
    static_cast<void>(std::snprintf(
        text.data(), text.size(),
        "%s %s session=%" PRIu32 " samples[hb=%" PRIu64 ",diag=%" PRIu64
        ",motor=%" PRIu64 ",pwm=%" PRIu64 ",imu=%" PRIu64 ",battery=%" PRIu64
        ",button=%" PRIu64
        "] rates[hb=%.2f,diag=%.2f,motor=%.2f,"
        "pwm=%.2f,imu=%.2f,battery=%.2f] traffic_avg=%.1fB/s "
        "traffic_peak_interval=%.1fB/s zero_tx=%" PRIu64 " zero_window=%" PRIu64
        " motor_evidence[msg=%" PRIu64 ",consume=%" PRIu64 ",overwrite=%" PRIu64
        ",accounted=%" PRIu64 ",over20ms=%" PRIu64 ",max_age_us=%" PRIu32
        "]"
        " failures=0x%016" PRIx64
        " masks[missing=0x%02x,lost=0x%02x,rate=0x%02x,stamp=0x%02x,"
        "invalid=0x%02x]",
        mode, summary.passed ? "PASS" : "FAIL", summary.agent_session_id,
        summary.streams[0].sample_count, summary.streams[1].sample_count,
        summary.streams[2].sample_count, summary.streams[3].sample_count,
        summary.streams[4].sample_count, summary.streams[5].sample_count,
        summary.streams[6].sample_count, summary.streams[0].observed_rate_hz,
        ...
        summary.streams[5].observed_rate_hz, summary.transport_bytes_per_second,
        ...
        summary.motor_command_age_over_20_ms, summary.motor_command_max_age_us,
        summary.failure_mask,
