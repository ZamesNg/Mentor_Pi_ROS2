...
## Component-layout and tracking-controller gates
...
ROS-to-steady clock conversion, stale pose, MPC fallback, and every
zero-output condition. `VER-TRACK-INT-001` supplies synthetic map poses and
...
| `VER-TRACK-UNIT-001` | Unit tests cover polynomial evaluation and numerical derivatives, exact terminal-pose clamping with zero derivatives, map-frame message validation, all four plugin exports, direct geometry-center map-pose state ingestion, Mecanum unwrapped-yaw tracking, Ackermann geometry-center dynamics with no yaw objective, trajectory LADRC initialization/evolution/reset and post-bound ESO input, stationary MPC prediction beyond the execution horizon, geometry/profile limits including every Mecanum wheel and Ackermann steering bound, scheduling and cancellation, ROS-to-steady conversion, stale state, MPC bounded fallback, failure/deadline handling, and all zero-output conditions. |
| `VER-TRACK-INT-001` | Integration tests supply only synthetic geometry-center `PoseStamped` messages and scheduled map-frame trajectories to the single `/mentor_pi/trajectory_tracker` contract for both vehicles and algorithms; verify trajectory, controller-reference, cancel, frame, tracker-owned diagnostic, static wheel-limit, replacement, and terminal-hold contracts; reject MCU motor-state, heartbeat, authorization, and odometry graph dependencies; observe bounded forward, reverse, and endpoint-correction output; prove stale pose temporarily inhibits and then resumes terminal hold; and prove cancellation returns output to zero. |
...
| `VER-POSE-MAP-001` | Unit, runtime, and launch-contract tests verify exact `map` input, finite pose validation, quaternion normalization, the `/vrpn_mocap/<robot>/pose` convention, common `vehicle/pose`, and `map -> <robot>/base_footprint`; reject non-map and nonfinite mocap; require controller `enable_odom_tf=false`; keep controller odometry on internal remapped topics; reject public `vehicle/odometry`, `vehicle/tf_odometry`, and an `odom` TF root; preserve the Ackermann `base_footprint -> rear_axle_footprint` offset, wheel links, and `imu_link`; and prove tracker ingestion applies no second shift. |
| `VER-SIM-001` | Plugin unit tests cover exact interfaces, lifecycle transitions, finite saturation, wheel acceleration ramps and reversals, trapezoidal position integration, Ackermann steering averaging/rate/angle limits, invalid inputs, and deactivation. Headless launch tests start both vehicle types with nonzero initial geometry-center poses in `map`; require `vehicle` active; verify tracker-disabled signed direct control and reference timeout; and run trajectory-driven Ackermann/Mecanum ADRC/MPC motion using only public map pose. Graph tests require the default tracker and reject Agent, supervisor, firmware, motor-state, heartbeat, authorization, and public odometry endpoints. Launch-contract tests verify simulation-only URDF plugins, common tracker selection, initial pose, separate Foxglove bridge, and absence of `/clock`. |
...
| `VER-CONFIG-001` | The non-root `mentor-pi-agent.service` is the sole `/dev/mentor_pi_mcu` owner, starts at boot, restarts after serial loss, uses a versioned verified Agent, and never starts ROS applications. Manual `controller.launch.py` starts only the supervisor and shuts its containing application launch down on supervisor exit. The supervisor accepts every valid YAML boundary and rejects unknown keys, wrong types/counts, invalid models, and adjacent out-of-range values while keeping authorization zero. Exact response-profile and generation/session retry tests reject inconsistent, late, stale, failed, or exhausted configuration, reapply in order after new sessions, and replay no actuator or bus state. |
