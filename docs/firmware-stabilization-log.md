...
| --- | --- | --- | --- | --- | --- |
| HOST-SERIAL-001 | Corrected; hardware observed | `make flash` rejected the CH9102F stable path as unreadable/unwritable. | The login user lacked the project serial group and udev alias. | `tools/configure_dev_serial_access.sh` creates `/dev/mentor_pi_mcu`, configures `mentor-pi-serial`, and adds the selected user. `newgrp mentor-pi-serial` opens a shell with membership immediately. | Helper tests and `make doctor` validate identity, alias, ownership, and permissions. |
