...
On native Ubuntu 22.04, an already built workspace can instead use the same
validated launch directly. First run `make host agent`, enter `make shell`,
then run inside that sourced shell:
...
The Python launch performs the development artifact and serial preflight
before starting the Agent and supervisor. It shuts down the complete launch if
either process exits. Other Ubuntu releases still use `make start` because the
pinned Humble environment lives inside its runtime container.
...
**Warning:** Use `make start`, not a ros2_control vehicle launch, for this
section. Verify that no other process publishes `/mentor_pi/motors/command`.
