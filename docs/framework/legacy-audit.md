...
|---|---|---|---|---|---|
| ROS-LED-01 | `~/set_led` (`LedState`) → function 1, `id,on_ms,off_ms,repeat` | `packet_led_handle()` → LED pattern timer → PD9/PD10; PD11 also indicates packets | `leds/command` (`LedCommand`), all three channels | Only IDs 1–2 are accepted although three LEDs exist; payload size is unchecked; float seconds silently narrow to 16-bit milliseconds; LED3 ownership conflicts with packet indication. v2 validates first, owns all three explicitly, and never uses a user LED as hidden transport state. | `VER-HIL-LED-001`, `VER-UNIT-VAL-001` |
...
|---|---|---|---|---|
| HW-TRN-01 | Data USB-C → CH9102F → PA9/PA10 USART1 at 1,000,000 baud, 8N1; normal-mode DMA/custom frames | **Replace** with micro-ROS XRCE serial, 8 KiB circular RX DMA, 1 KiB TX bounce, MTU 512 | RX FIFO result is ignored; DMA receive is aborted/restarted in callbacks; overflow is silent; callbacks execute application work. The v2 above-BASEPRI RX top half counts every half/full epoch without FreeRTOS calls, and the deferred IRQ only transfers bounded state and notifies `MicroRosTask`. | `VER-UNIT-RXDMA-001`, `VER-INT-TRN-001`, `VER-FUZZ-TRN-001`, `VER-RECONNECT-USB-001` |
| HW-MOT-01 | Four YX-4055AM outputs, four quadrature encoders, 100 Hz PID | **Replace** through v2 motor APIs, with normal images locked to zero/stop until HIL | In addition to ROS-MOT-01, output must be safe before driver initialization and on lease/session failure. Passive encoder direction must be proven before the acknowledged 0.25 RPS/300 permille commissioning image is used; commissioning evidence does not itself qualify PID or polarity for release. | `VER-BUILD-MOTOR-GATE-001`, `VER-HIL-MOT-001`, `VER-SAFE-LEASE-001` |
...
| HW-LED-01 | LEDs 1 and 2 are active-low on PD9/PD10; LED3 is active-high on PD11 | **Replace** through `leds/command` | LED3 is implicitly flashed for packets, conflicting with user ownership. v2 has no hidden transport indication. | `VER-HIL-LED-001` |
...
| HW-RGB-01 | Two WS2812-compatible pixels through SPI1; function 11 → `packet_RGB_Ctl_handle()` | **Replace** through `rgb/command`, exactly two pixels | No legacy ROS endpoint; pixel mask/payload are unchecked; Python function enum is inconsistent. v2 validates `update_mask` and fixed arrays. | `VER-HIL-RGB-001`, `VER-UNIT-VAL-001` |
