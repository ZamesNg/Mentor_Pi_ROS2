...
| MOTOR-SIGN-COORDINATE-001 | Corrected in source; deployment pending | Commit `9b087ea` introduced separate encoder/output model signs while the host map remained unchanged. On `ackermann_0`, forward manual rear-wheel rotation then appeared negative on both ROS joints. | Multiple sign layers split target, measurement, and output across inconsistent coordinates. The legacy negative PID gains were not sufficient evidence for an independent v2 bridge inversion. | All firmware sign transforms are removed: MCU encoder state equals raw signed delta and bridge duty equals LADRC output for every model. The single host ROS↔MCU map is `{-1,+1,-1,+1}` in FL,FR,RL,RR order. Focused tests fix raw state, direct output, and bidirectional host mapping. | Flash and repeat passive sign checks per vehicle. Powered direction and ADRC behavior remain unqualified. |
