...
constexpr std::size_t kOledPeripheralIndex = 2U;
constexpr std::uint8_t kIoErrorResult = 5U;
constexpr std::uint8_t kOledErrorSource = 8U;
constexpr std::uint16_t kOledInitializationDetail = 1U;
...
bool IsAbsentQualificationBatteryState(std::uint16_t voltage_mv,
  ...
  return !valid && voltage_mv == 0U && !below_threshold &&
         low_threshold_mv >= kMinimumBatteryThresholdMv &&
         low_threshold_mv <= kMaximumBatteryVoltageAndThresholdMv;
}
...
QualificationMonitorCore::QualificationMonitorCore(
  ...
  if (configuration_.duration_ns <= 0 ||
      ...
      (configuration_.allow_missing_oled &&
...
void QualificationMonitorCore::ObserveHeartbeat(
  ...
  if (observation.state == kHeartbeatReady) {
  ...
  if (configuration_.imu_characterization_mode && !observation.imu_healthy) {
...
void QualificationMonitorCore::ObserveDiagnostics(
  ...
  if (configuration_.imu_characterization_mode) {
    last_diagnostics_match_first_board_mode_ =
        DiagnosticsMatchFirstBoardMode(observation);
    const bool discovery_complete =
        observation.arrival_time_ns - configuration_.start_time_ns >=
        configuration_.discovery_timeout_ns;
    if (discovery_complete && !last_diagnostics_match_first_board_mode_) {
      AddFailure(QualificationFailure::kImuCharacterizationMismatch);
    }
  }
  ...
  if (!diagnostics_seen_) {
    ...
    baseline_diagnostic_fault_total_ =
        EffectiveDiagnosticFaultTotal(observation);
    ...
    if (EffectiveDiagnosticFaultTotal(observation) !=
        baseline_diagnostic_fault_total_) {
...
QualificationSummary QualificationMonitorCore::Finish(
  ...
  if (!heartbeat_seen_ || !heartbeat_progress_seen_ || !heartbeat_ready_seen_ ||
      last_heartbeat_state_ != kHeartbeatReady) {
    AddFailure(QualificationFailure::kHeartbeatNotReady);
  }
  if (configuration_.imu_characterization_mode) {
    if (!last_heartbeat_imu_healthy_) {
      AddFailure(QualificationFailure::kImuCharacterizationMismatch);
    }
    ...
    if (!diagnostics_seen_ || !last_diagnostics_match_first_board_mode_) {
      AddFailure(QualificationFailure::kImuCharacterizationMismatch);
    }
  }
...
bool QualificationMonitorCore::DiagnosticsMatchFirstBoardMode(
    const DiagnosticsObservation& observation) const {
  if (observation.transport_error_total != 0U ||
      observation.peripheral_errors[kImuPeripheralIndex] != 0U) {
    return false;
  }
  const std::uint32_t oled_errors =
      observation.peripheral_errors[kOledPeripheralIndex];
  const bool missing_oled_allowed =
      configuration_.allow_missing_oled && oled_errors > 0U &&
      observation.peripheral_timeouts[kOledPeripheralIndex] == 0U;
  for (std::size_t index = 0U; index < observation.peripheral_errors.size();
       ++index) {
    if ((index != kOledPeripheralIndex &&
         observation.peripheral_errors[index] != 0U) ||
        observation.peripheral_timeouts[index] != 0U) {
      return false;
    }
  }
  if (!missing_oled_allowed && oled_errors != 0U) {
    return false;
  }
  const std::uint64_t expected_fault_total =
      static_cast<std::uint64_t>(oled_errors);
  if (observation.diagnostic_fault_total != expected_fault_total) {
    return false;
  }
  if (!missing_oled_allowed) {
    return true;
  }
  return observation.last_error_code == kIoErrorResult &&
         observation.last_error_source == kOledErrorSource &&
         observation.last_error_detail == kOledInitializationDetail;
}

std::uint64_t QualificationMonitorCore::EffectiveDiagnosticFaultTotal(
    const DiagnosticsObservation& observation) const {
  if (!configuration_.allow_missing_oled) {
    return observation.diagnostic_fault_total;
  }
  const std::uint64_t oled_errors =
      observation.peripheral_errors[kOledPeripheralIndex];
  return observation.diagnostic_fault_total >= oled_errors
             ? observation.diagnostic_fault_total - oled_errors
