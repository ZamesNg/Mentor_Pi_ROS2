...
package and generated development workspace requests for unused `clang-tidy`
are excluded from this production build because analysis and fuzzing are
intentionally not onboard gates; all dependencies used by the installer and
`generate_lib` workflow are still resolved. It also verifies the checked-in
