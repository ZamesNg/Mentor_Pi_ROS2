# Normal Computer Tutorial 07: Run Full Software, Stress, Soak, and Release Gates
...
**Run on:** the normal computer for the complete pinned-container software
suite, then the connected Docker Humble runtime for physical campaigns
...
## 1. Run the complete software gates

```sh
cd /home/zames/Mentor_Pi && make release-software-gates
```

Type `RUN_RELEASE_SOFTWARE_GATES`. The helper runs documentation, native
...
This track records the required Clang 18 fuzz and cross-target analysis
evidence. The onboard-computer track does not claim those container gates.
...
Run architecture gates on native `amd64` and native `arm64`; do not substitute
slow cross-architecture emulation.
