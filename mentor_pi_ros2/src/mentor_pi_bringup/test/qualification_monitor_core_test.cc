...
using mentor_pi_bringup::IsAbsentQualificationBatteryState;
...
constexpr std::size_t kOledErrorCounterIndex = 35U;
...
struct CharacterizationOptions {
  ...
  bool heartbeat_imu_healthy = true;
  bool imu_valid = true;
  bool imu_vectors_finite = true;
  bool imu_nonzero_stamp = true;
  std::size_t imu_sample_count = 3001U;
  std::uint8_t heartbeat_state = 1U;
  std::uint8_t last_error_code = 0U;
  std::uint8_t last_error_source = 0U;
  std::uint16_t last_error_detail = 0U;
  std::uint32_t imu_error_count = 0U;
...
QualificationConfiguration CharacterizationConfiguration(
  ...
  configuration.allow_missing_oled = allow_missing_oled;
  return configuration;
}
...
DiagnosticsObservation CharacterizationDiagnosticsAt(
  ...
  observation.monotonic_counters[kImuErrorCounterIndex] =
      options.imu_error_count + options.imu_timeout_count;
  observation.monotonic_counters[kOledErrorCounterIndex] =
      options.non_imu_error_count + options.non_imu_timeout_count;
...
void PopulateCharacterizationRun(
    QualificationMonitorCore* core, const CharacterizationOptions& options = {},
    const CharacterizationOptions* settling_diagnostics = nullptr) {
  ObserveAllPublishers(core);
  ...
  for (std::int64_t time_ns = 0; time_ns <= kTestDuration;
       time_ns += kNanosecondsPerSecond) {
    const CharacterizationOptions& diagnostics_options =
        settling_diagnostics != nullptr && time_ns < 5 * kNanosecondsPerSecond
            ? *settling_diagnostics
            : options;
    core->ObserveDiagnostics(
        CharacterizationDiagnosticsAt(time_ns, diagnostics_options));
    core->ObserveTelemetry(QualificationStream::kBattery, StampAt(time_ns),
                           time_ns, true);
  }
  for (std::int64_t time_ns = 0; time_ns <= kTestDuration;
  ...
  for (std::size_t index = 0U; index < options.imu_sample_count; ++index) {
    const std::int64_t time_ns =
        static_cast<std::int64_t>(index) * 20 * kNanosecondsPerMillisecond;
    ImuObservation imu;
    if (options.imu_nonzero_stamp) {
      imu.stamp = StampAt(time_ns);
    }
    imu.arrival_time_ns = time_ns;
...
void TestPassingImuCharacterizationRun() {
  ...
  Expect(summary.passed, "healthy first-board characterization run passes");
  ...
  Expect(summary.streams[static_cast<std::size_t>(QualificationStream::kImu)]
                 .sample_count == 3001U,
         "characterization requires the complete 50 Hz IMU stream");
  Expect(
      (summary.invalid_telemetry_mask &
       (UINT8_C(1) << static_cast<std::uint8_t>(QualificationStream::kImu))) ==
          0U,
      "valid IMU samples do not set the invalid telemetry mask");
  ...
  Expect(!late_join_summary.passed,
         "characterization rejects a missing IMU stream");
  Expect(HasFailure(late_join_summary.failure_mask,
                    QualificationFailure::kRateOutOfRange),
         "missing IMU samples report a rate failure");
  ...
  Expect(strict_summary.passed,
         "strict core accepts the same healthy graph and telemetry evidence");
}

void TestCharacterizationWithDeclaredMissingOled() {
  CharacterizationOptions options;
  options.non_imu_error_count = 5U;
  options.last_error_code = 5U;
  options.last_error_source = 8U;
  options.last_error_detail = 1U;

  QualificationMonitorCore core(CharacterizationConfiguration(true));
  PopulateCharacterizationRun(&core, options);

  CharacterizationOptions later = options;
  later.non_imu_error_count = 9U;
  core.ObserveDiagnostics(CharacterizationDiagnosticsAt(
      kTestDuration + kNanosecondsPerSecond, later));
  const auto summary = core.Finish(kTestDuration + kNanosecondsPerSecond);
  Expect(summary.passed,
         "declared absent OLED accepts only its repeated initialization NACK");

  CharacterizationOptions stale_teardown = options;
  stale_teardown.last_error_code = 2U;
  stale_teardown.last_error_source = 1U;
  stale_teardown.last_error_detail = 7U;
  QualificationMonitorCore settling_core(CharacterizationConfiguration(true));
  PopulateCharacterizationRun(&settling_core, options, &stale_teardown);
  const auto settling_summary = settling_core.Finish(kTestDuration);
  Expect(settling_summary.passed,
         "a stale Agent teardown last-error may settle during discovery");

  QualificationMonitorCore strict_core(CharacterizationConfiguration());
  PopulateCharacterizationRun(&strict_core, options);
  const auto strict_summary = strict_core.Finish(kTestDuration);
  Expect(!strict_summary.passed,
         "default characterization still rejects an OLED failure");

  CharacterizationOptions timeout = options;
  timeout.non_imu_timeout_count = 1U;
  QualificationMonitorCore timeout_core(CharacterizationConfiguration(true));
  PopulateCharacterizationRun(&timeout_core, timeout);
  const auto timeout_summary = timeout_core.Finish(kTestDuration);
  Expect(!timeout_summary.passed,
         "declared absent OLED does not excuse an I2C timeout");

  QualificationConfiguration invalid_configuration;
  invalid_configuration.allow_missing_oled = true;
  QualificationMonitorCore invalid_core(invalid_configuration);
  const auto invalid_summary = invalid_core.Finish(kTestDuration);
  Expect(HasFailure(invalid_summary.failure_mask,
                    QualificationFailure::kInvalidConfiguration),
         "strict preflight cannot enable the missing-OLED exception");
}

void ExpectCharacterizationFailure(const CharacterizationOptions& options,
...
void TestImuCharacterizationRejectsAdjacentStates() {
  CharacterizationOptions options;
  options.heartbeat_imu_healthy = false;
  ExpectCharacterizationFailure(options, "missing IMU_HEALTHY heartbeat flag");

  options = {};
  options.heartbeat_state = 2U;
  ExpectCharacterizationFailure(options, "DEGRADED heartbeat");

  options = {};
  options.heartbeat_state = 3U;
  ExpectCharacterizationFailure(options, "FAULT heartbeat");

  options = {};
  options.time_synchronized = false;
  ExpectCharacterizationFailure(options, "unsynchronized heartbeat");

  options = {};
  options.imu_valid = false;
  ExpectCharacterizationFailure(options, "invalid IMU sample");

  options = {};
  options.imu_vectors_finite = false;
  ExpectCharacterizationFailure(options, "non-finite IMU sample");

  options = {};
  options.imu_sample_count = 1U;
  ExpectCharacterizationFailure(options, "underspeed IMU stream");

  options = {};
  options.imu_nonzero_stamp = false;
  ExpectCharacterizationFailure(options, "zero IMU timestamp");

  options = {};
  options.imu_timeout_count = 1U;
  ExpectCharacterizationFailure(options, "IMU timeout");

  options = {};
  options.non_imu_error_count = 1U;
  ExpectCharacterizationFailure(options, "non-IMU peripheral error");

  options = {};
  options.non_imu_timeout_count = 1U;
  ExpectCharacterizationFailure(options, "non-IMU peripheral timeout");

  options = {};
  options.imu_error_count = 1U;
  ExpectCharacterizationFailure(options, "IMU error");
}
...
void TestBatteryContractBoundaries() {
  ...
  Expect(IsAbsentQualificationBatteryState(0U, 6300U, false, false),
         "exact USB-only battery state is recognized");
  Expect(!IsAbsentQualificationBatteryState(1U, 6300U, false, false),
         "nonzero invalid voltage is not the absent-battery state");
  Expect(!IsAbsentQualificationBatteryState(0U, 6300U, true, false),
         "valid flag is not the absent-battery state");
  Expect(!IsAbsentQualificationBatteryState(0U, 6300U, false, true),
         "an absent battery cannot be below threshold");
}
...
int main() {
  ...
  TestCharacterizationWithDeclaredMissingOled();
