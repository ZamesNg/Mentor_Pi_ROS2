...
**Run on:** RDK X5 using the bounded onboard Docker gate
...
## 1. Run the RDK onboard software gate
...
The standard `make rdk-handoff` workflow produces the deployable arm64 image,
but it does not satisfy this native gate. Image loading, runtime, memory,
tracker-deadline, and applicable hardware gates still run on the RDK before
release or performance claims. The omitted full-computer checks remain
separate host-computer evidence.

## 2. Run the connected zero-command preflight

Leave `mentor-pi-controller.target` active with the same verified PID candidate.
