...
| `VER-SIM-001` | Plugin unit tests cover exact interfaces, lifecycle transitions, finite saturation, wheel acceleration ramps and reversals, trapezoidal position integration, Ackermann steering averaging/rate/angle limits, invalid inputs, and deactivation. Headless launch tests start both vehicle types with nonzero initial geometry-center poses; require `vehicle` active; verify signed forward, lateral, turning, reverse, and reference-timeout behavior through public odometry; and reject Agent, supervisor, firmware, and tracker graph endpoints. Launch-contract tests verify the simulation-only URDF plugins, common API, separate Foxglove bridge, and absence of `/clock`. |
