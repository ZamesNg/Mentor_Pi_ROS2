...
`RRCLITE_MOTOR_COMMISSIONING_ACK=MOTORS_RAISED`. Its initial direction-check
mode shall use fixed 250-permille output selected only by command sign, bypass
PID, admit no command above 0.25 RPS, and disarm above 0.50 measured RPS. The
acknowledgement, raised wheels, and caps reduce commissioning risk; none is
evidence that any PID profile or regulated speed is release-qualified.
...
| `VER-BUILD-HOST-001` | On native Ubuntu 22.04 with ROS 2 Humble, and from the pinned Ubuntu 22.04/Humble container on another Ubuntu development host, debug, release, ASan/UBSan, and TSan jobs build and test all first-party host C++, generated interfaces, `mentor_pi_bringup`, and `mentor_pi_hardwares` with zero first-party warnings. Generated build metadata records `ubuntu=22.04`, `ros_distro=humble`, and the automatically detected `amd64` or `arm64` architecture and rejects an OS, distribution, architecture, source-fingerprint, or metadata-schema mismatch. The non-22.04 runtime passes only the reviewed CH9102F character device and never uses a privileged container. Project-owned production nodes and the control/data path contain no Python node, script, `rclpy` use, or serial translation bridge; ROS launch files may use the upstream Python launch framework. Pinned upstream ROS tools and their Python implementation/dependencies are permitted and reported separately. |
...
| `VER-BUILD-MOTOR-GATE-001` | For SAFE-006, `make firmware` reports `LOCKED` and compiles nonzero-motion authority out. `make firmware-commissioning` and `make firmware-commissioning-pid` fail without the exact `COMMISSIONING_BUILD_ACK=MOTORS_RAISED` value; direct CMake configuration with commissioning on and a missing/wrong internal acknowledgement also fails. The direction-check target reports `COMMISSIONING`, `control_mode=DIRECTION_CHECK`, fixed 250-permille output, a 0.50 RPS overspeed cutoff, and a 0.25 RPS command-admission bound. The PID target reports `COMMISSIONING_PID`, `control_mode=CLOSED_LOOP`, a 6 RPS implementation ceiling, and a 1000-permille output clamp. Artifact and flash tests classify all three modes independently and reject cross-mode substitution, stale metadata, changed source/dependency/artifact hashes, mutated snapshots, missing or wrong acknowledgements, programmer failure, and missing read-back success. Valid commissioning flashes additionally require `COMMISSIONING_FLASH_ACK=MOTORS_RAISED_CURRENT_LIMITED`. Neither commissioning artifact is release-qualified. |
...
| `VER-HIL-MOT-001` | `ROS-MOT-01`, `HW-MOT-01` | The test is staged. First, flash the normal locked image, prove a selected nonzero command is rejected while zero/stop succeeds, keep all bridge outputs at zero, manually rotate each raised wheel in the declared positive direction, and record raw counter and normalized `motors/state` signs. Second, use only the dual-acknowledged direction-check image, raised-wheel/equivalent guarding, and a current-limited supply; exercise one channel and one sign at a time, prove fixed output is 250 permille, require encoder and operator-observed physical direction to agree, verify the 0.50 RPS cutoff and stop/lease/session kill paths, and stop on wrong sign, unexpected current, oscillation, or cross-channel response. These stages characterize initial polarity only; they do not tune PID or prove RPM regulation. A later reviewed candidate and recorded HIL must qualify or replace every channel/model output sign, encoder constant, PID/filter/deadband profile, full documented RPS range, subset merge, zero behavior, model-change behavior, and independent state before production motion is released. |
...
Before full motor release qualification, this run shall use a later qualified
closed-loop commissioning image on the guarded/current-limited fixture. The
initial fixed-output direction-check image is not suitable for this load run. A normal locked-image
...
[Tutorial 08](../tutorials/08-run-stress-soak-and-release-gates.md) as the
