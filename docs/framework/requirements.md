...
| ID | Requirement | Verification |
|---|---|---|
| HOST-001 | A project-owned C++17 `rclcpp` node named `/mentor_pi/configuration_supervisor` **shall** validate the exact YAML schema in [architecture.md](architecture.md) for motor model, twelve per-motor nonlinear ADRC/filter arrays, two directional minimum-drive arrays, four PWM-servo offsets, and battery threshold. Invalid configuration shall keep project-owned host motion disabled. | Inspection, Test |
...
| HOST-005 | Retired. Global pose, trajectory messages, and outer trajectory tracking are owned by external applications. | Inspection |
| HOST-006 | The mecanum and Ackermann hardware plugins **shall** apply first-order ADRC at the 50 Hz hardware loop to all accepted manual or tracker commands. Wheel feedback shall supply translation/longitudinal speed and `/mentor_pi/imu` gyroscope Z shall supply yaw rate; acceleration shall not be integrated. Missing, invalid, or older-than-100-ms required feedback shall command zero/center and fail through the existing authorized hardware error path. Ackermann yaw control shall reset and center steering below 0.1 m/s measured speed. | Inspection, Test |
| HOST-007 | Physical and simulation launch **shall** expose no project-owned global-pose or TF adapter. Controller odometry shall remain on private remapped `vehicle/_controller_*` topics and controller odometry TF shall remain disabled. External applications exclusively own global pose and `map -> <robot>/base_footprint`. `rear_axle_footprint`, wheel links, and `imu_link` shall remain in the URDF subtree (`VER-POSE-MAP-001`). | Inspection, Test |
| HOST-008 | Development simulation **shall** provide actuator-limited Ackermann and Mecanum `ros2_control` hardware plugins using the production stamped-command controller, URDF visuals, namespace, and private controller odometry. It shall use deterministic 50 Hz wall time, consume the latest best-effort reference at that rate, and enforce the 100 ms reference timeout from local receipt time regardless of a zero, stale, current, or unsynchronized sender stamp, without `/clock`, Agent, supervisor, firmware endpoints, noise, slip, collision, terrain, or IMU models (`VER-SIM-001`). | Inspection, Test |
...
| TRN-003 | A loss of the Agent or USB transport **shall** be detected, shall disarm the DC motors, and shall enter the reconnect state without rebooting the MCU. `WAIT_AGENT` shall retain bounded Agent probing, while `ACTIVE` shall perform no Agent ping and shall instead treat a failed or timed-out reliable 2 Hz heartbeat acknowledgement as Agent loss. Under the supported traffic profile, ACTIVE Agent loss shall be detected within 550 ms. | Test |
...
| RT-002 | A micro-ROS callback **shall not** wait on a peripheral transaction, mutex owned by a slower worker, or an unbounded queue operation. The ACTIVE executor spin shall use a zero timeout while a notification-aware outer task wait is bounded to 1 ms. Reliable publish/response operations and incremental ROS entity create/finalize operations shall use the bounded middleware timeouts in [architecture.md](architecture.md), so the micro-ROS task heartbeat interval never exceeds 100 ms. | Inspection, Analysis, Test |
...
| RT-008 | Each ACTIVE executor iteration **shall** start at most one 10 ms blocking operation across service response, reliable telemetry, and time synchronization. Service, reliable-telemetry, and maintenance opportunities shall rotate without starvation. Each iteration shall publish at most one due best-effort telemetry sample; no blocking operation shall start while best-effort telemetry is overdue or with less than 12 ms until the next motor, PWM, or IMU publication deadline. Heartbeat, battery, and diagnostics shall use the wrap-safe ACTIVE-entry phases defined in [architecture.md](architecture.md). | Inspection, Analysis, Test |
...
| SAFE-006 | `make -C firmware build`, `verify`, `package`, `flash`, and `flash-package` **shall** select only the `NORMAL_CLOSED_LOOP_DEFAULT` ADRC artifact with `control_mode=CLOSED_LOOP`; no alternate motor-authority mode or runtime unlock is supported. Package flashing shall require exact equality with the intended robot namespace and a separately recorded digest of the transferred manifest before bootloader activation. The controller shall enforce the active model's RPS limit and the 6 RPS implementation ceiling, clamp output to +/-1000 permille, reject an invalid motor command atomically without refreshing any selected lease, and require the session-bound host configuration gate before project-owned host motion starts. | Inspection, Test |
