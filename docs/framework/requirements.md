...
| HOST-005 | The hardware-independent plugin-based tracker **shall** implement the scheduling, validation, vehicle model, deadline, ADRC/MPC selection, fallback, static actuator bounds, and zero-output behavior in [tracking-controller.md](tracking-controller.md). Physical and simulation vehicle launches shall default to the vehicle-matched ADRC plugin; an explicit launch selection may choose MPC or disable the tracker. Both algorithms and both vehicle types shall use the same node and endpoint names. Its pose state shall be `(x_center, y_center, yaw)` copied from fresh geometry-center vehicle odometry. It shall not subscribe to MCU motor state, heartbeat, authorization, Agent, or configuration-supervisor interfaces, and shall not perform high-level planning or arbitrary frame transforms (`VER-TRACK-UNIT-001`, `VER-TRACK-INT-001`, `VER-TRACK-RDK-001`). | Inspection, Test |
