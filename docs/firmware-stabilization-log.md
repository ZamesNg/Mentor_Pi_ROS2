...
| BOARD-PROFILE-001 | Encoder mapping verified; earlier M1 powered result superseded | Passive one-wheel-at-a-time capture established `front-left=M1`, `front-right=M3`, `rear-left=M2`, and `rear-right=M4`. Raw forward deltas were M1 `+742`, M2 `+1915`, M3 `-1370`, and M4 `-1612`. A 100 ms M1 run ended at only `-1` tick and failed closed; the repeat at `+0.25 RPS` for 2,000 ms produced `+294` ticks, peak magnitude 0.452010 RPS, confirmed zero, and passed. | Connector ownership follows the confirmed chassis layout, while right-side encoder wiring has the opposite raw sign. The isolated one-tick short-window result was noise/settling, not reliable polarity evidence. The longer run predates the later encoder/drive sign separation and no longer qualifies the current mapping. | The maintained profile is `docs/framework/verified-hardware-profile.md`; the composition root uses channel signs `{+1,+1,+1,+1}` and the host owns chassis-side inversion. | Repeat guarded drive-direction verification for every channel before powered use. |
| MOTOR-POLARITY-SPLIT-001 | Corrected in source; Ackermann 0 passive verification pending | JGA27 encoder state was multiplied by `-1`, while its bridge output was not inverted. The host already owns the left/right chassis conversion, so the firmware state contradicted both the legacy raw-counter implementation and the recorded passive forward deltas. | The negative legacy JGA27 PID gains were incorrectly treated as evidence about encoder direction. They instead represented the inverted relationship between semantic controller output and the physical bridge. | Every model now retains encoder factor `+1`; JGA27 alone applies drive-output factor `-1` at the hardware boundary. The LADRC observer remains in semantic target/measurement coordinates. | Focused domain tests cover raw count direction, channel wiring multiplication, JGA27 negative bridge output, and conventional-model positive bridge output. Flash Ackermann 0, repeat passive rear-wheel direction checks, then perform guarded powered verification separately. |
