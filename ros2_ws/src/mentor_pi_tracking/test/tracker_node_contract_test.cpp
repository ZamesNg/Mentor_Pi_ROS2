...
void ExerciseTracker(VehicleType vehicle, const std::string& controller,
  ...
  const auto supervisor = std::make_shared<rclcpp::Node>(
      "configuration_supervisor", "/mentor_pi");
  ...
  constexpr std::uint32_t kAgentSession = 42U;
  authorization.data = (std::uint64_t{1} << 32U) | kAgentSession;
  mentor_pi_interfaces::msg::Heartbeat heartbeat;
  heartbeat.agent_session_id = kAgentSession;
  ...
  {
    const auto rogue_authorization_publisher =
        peer->create_publisher<std_msgs::msg::UInt64>(
            "/mentor_pi/configuration/motion_authorization",
            rclcpp::QoS(rclcpp::KeepLast(1)).reliable().transient_local());
    command.reset();
    bool duplicate_publisher_zero = false;
    const auto duplicate_deadline =
        std::chrono::steady_clock::now() + std::chrono::milliseconds(300);
    while (!duplicate_publisher_zero &&
           std::chrono::steady_clock::now() < duplicate_deadline) {
      rogue_authorization_publisher->publish(authorization);
      authorization_publisher->publish(authorization);
      heartbeat_publisher->publish(heartbeat);
      motor_publisher->publish(motor);
      odometry_publisher->publish(odometry);
      executor.spin_some();
      duplicate_publisher_zero =
          command != nullptr && command->twist.linear.x == 0.0 &&
          command->twist.linear.y == 0.0 && command->twist.angular.z == 0.0;
      std::this_thread::sleep_for(std::chrono::milliseconds(10));
    }
    ASSERT_TRUE(duplicate_publisher_zero);
  }

  std_msgs::msg::UInt64 mismatched_authorization = authorization;
  mismatched_authorization.data = (std::uint64_t{1} << 32U) | 43U;
  command.reset();
  bool mismatched_session_zero = false;
  const auto mismatch_deadline =
      std::chrono::steady_clock::now() + std::chrono::milliseconds(300);
  while (!mismatched_session_zero &&
         std::chrono::steady_clock::now() < mismatch_deadline) {
    authorization_publisher->publish(mismatched_authorization);
    heartbeat_publisher->publish(heartbeat);
    motor_publisher->publish(motor);
    odometry_publisher->publish(odometry);
    executor.spin_some();
    mismatched_session_zero =
        command != nullptr && command->twist.linear.x == 0.0 &&
        command->twist.linear.y == 0.0 && command->twist.angular.z == 0.0;
    std::this_thread::sleep_for(std::chrono::milliseconds(10));
  }
  ASSERT_TRUE(mismatched_session_zero);
