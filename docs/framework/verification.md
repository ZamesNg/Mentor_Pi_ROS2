...
| `VER-TRACK-UNIT-001` | Unit tests cover polynomial evaluation and numerical derivatives, exact terminal-pose clamping with zero derivatives, bounded message validation, all four plugin exports, direct geometry-center odometry state ingestion, Mecanum unwrapped-yaw tracking, Ackermann geometry-center dynamics with no yaw objective, trajectory LADRC initialization/evolution/reset and post-bound ESO input, stationary MPC prediction beyond the execution horizon, geometry/profile limits including every Mecanum wheel and Ackermann steering bound, scheduling and cancellation, ROS-to-steady conversion, stale state, MPC bounded fallback, failure/deadline handling, and all zero-output conditions. |
| `VER-TRACK-INT-001` | Integration tests supply only synthetic geometry-center odometry and scheduled trajectories to the single `/mentor_pi/trajectory_tracker` contract for both vehicles and algorithms; verify trajectory, controller-reference, cancel, frame, tracker-owned diagnostic, static wheel-limit, replacement, and terminal-hold contracts; reject MCU motor-state, heartbeat, and authorization graph dependencies; observe bounded forward, reverse, and endpoint-correction output; prove stale odometry temporarily inhibits and then resumes terminal hold; and prove cancellation returns output to zero. |
...
| `VER-ODOM-CENTER-001` | Unit and launch-contract tests verify yaw-aware Ackermann rear-axle-to-center pose, twist, TF, and covariance conversion; zero-offset Mecanum identity; invalid input rejection; hidden raw controller topics; public `vehicle/odometry` and `vehicle/tf_odometry`; standard `/tf`; namespaced `odom -> base_footprint`; the fixed Ackermann `base_footprint -> rear_axle_footprint` offset; and direct tracker ingestion of `(x_center, y_center, yaw)` without a second shift. |
| `VER-SIM-001` | Plugin unit tests cover exact interfaces, lifecycle transitions, finite saturation, wheel acceleration ramps and reversals, trapezoidal position integration, Ackermann steering averaging/rate/angle limits, invalid inputs, and deactivation. Headless launch tests start both vehicle types with nonzero initial geometry-center poses; require `vehicle` active; verify tracker-disabled signed direct control and reference timeout; and run trajectory-driven Ackermann/Mecanum ADRC/MPC motion using only public odometry. Graph tests require the default tracker and reject Agent, supervisor, firmware, motor-state, heartbeat, and authorization endpoints. Launch-contract tests verify simulation-only URDF plugins, common tracker selection, initial pose, separate Foxglove bridge, and absence of `/clock`. |
