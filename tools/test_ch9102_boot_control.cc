...
using mentor_pi::tools::BootMode;
using mentor_pi::tools::ModemBackend;
using mentor_pi::tools::ModemOperation;
...
void RequireSequence(BootMode mode,
  ...
  FakeBackend backend;
  Require(mentor_pi::tools::ExecuteBootSequence(mode, &backend),
...
int main() {
  ...
  FakeBackend failure;
  failure.fail_at = 2U;
  Require(!mentor_pi::tools::ExecuteBootSequence(BootMode::kBootloader,
                                                  &failure),
          "ioctl failure was not propagated");
  Require(failure.operations.size() == 2U,
          "sequence continued after ioctl failure");
  Require(!mentor_pi::tools::ExecuteBootSequence(BootMode::kApplication,
                                                  nullptr),
          "null backend was accepted");
  Require(mentor_pi::tools::ParseBootMode("bootloader") ==
              BootMode::kBootloader,
          "bootloader mode parsing failed");
  Require(mentor_pi::tools::ParseBootMode("application") ==
              BootMode::kApplication,
