...
Before any powered motor case, archive a normal build proving that nonzero
commands are locked, then manually rotate each raised wheel with drive outputs
disabled and record raw and normalized encoder direction. A powered
pre-qualification case shall use a current-limited supply and an image built
with both `RRCLITE_MOTOR_COMMISSIONING=1` and
`RRCLITE_MOTOR_COMMISSIONING_ACK=MOTORS_RAISED`. It shall never request more
than 0.25 RPS or permit more than 300 permille output. The acknowledgement,
raised wheels, and caps reduce commissioning risk; none is evidence that JGA27's
provisional `-1` model polarity or any PID profile is release-qualified.
...
| `VER-BUILD-HOST-001` | On supported Ubuntu 22.04 with ROS 2 Humble, and from the pinned Ubuntu 22.04/Humble container on a clean Ubuntu 24.04 development host, debug, release, ASan/UBSan, and TSan jobs build and test all first-party host C++ and generated interfaces with zero first-party warnings. Handoff metadata records `ubuntu=22.04`, `ros_distro=humble`, and the exact `amd64` or `arm64` target and rejects an OS, distribution, architecture, or older metadata-schema mismatch. Project-owned production nodes and the control/data path contain no Python node, script, `rclpy` use, or serial translation bridge. Pinned upstream ROS tools and their Python implementation/dependencies are permitted and reported separately. |
| `VER-BUILD-FW-001` | `make firmware` drives the authoritative CMake/Ninja graph. Two clean builds with the pinned Arm toolchain produce identical loadable sections. The target is STM32F407VET6 with the pinned HAL, FreeRTOS, and Humble-compatible micro-ROS client; 33 generated-workspace checkouts are detached at their checked-in Humble source-lock commits, the deferred `libyaml` checkout is required at the lock's 34th commit, and the temporary `geometry2/tf2_msgs` source is pinned separately. Missing or unexpected repositories fail the build, the archive matches its reviewed SHA-256, first-party warnings are errors, and ELF, BIN/HEX, map, metadata, and size reports are emitted. Firmware metadata records the Humble distribution identity and a current schema; verification rejects an older or other-distribution artifact. Inspection proves no supported target invokes an independent IDE build graph. |
| `VER-BUILD-MOTOR-GATE-001` | For SAFE-006, `make firmware` reports `LOCKED` and compiles closed-loop authority out. `make firmware-commissioning` without the exact `COMMISSIONING_BUILD_ACK=MOTORS_RAISED` value fails before producing artifacts; direct CMake configuration with commissioning on and a missing/wrong internal acknowledgement also fails. Supplying the exact value reports `COMMISSIONING` and produces an image whose compiled limits are exactly 0.25 RPS and 300 permille. With a fake CubeProgrammer, flash tests reject a missing/ambiguous port, missing/wrong `FLASH_ACK`, the wrong motor profile, stale metadata, changed source/dependency/artifact hashes, a mutated snapshot, programmer failure, and missing read-back success. A valid locked invocation snapshots and flashes the verified bytes; a commissioning invocation additionally requires `COMMISSIONING_FLASH_ACK=MOTORS_RAISED_CURRENT_LIMITED`. Commissioning artifacts are distinctly labelled/hashed and are never substituted for a release image. |
...
| `VER-CONFIG-001` | The production systemd unit directly executes the compiled Agent at its pinned install path rather than launching it through `ros2 run`. With no device override it selects `/dev/mentor_pi_mcu`; a supplied serial-device argument is honored, and only the native Agent opens it. The node `/mentor_pi/configuration_supervisor` accepts every valid boundary of the exact YAML schema and rejects unknown keys, wrong types/counts, invalid model strings, and adjacent out-of-range values while keeping its motion gate false. Each exact motor-model response profile passes; an `OK` response with a mismatched active model, tick count, adjacent representable finite speed, NaN, or infinity is converted to `IO_ERROR`, attempted once, and cannot enable motion. First discovery, graph/heartbeat reappearance, session-ID change, wrap-aware uptime discontinuity, and normal `uint32` uptime wrap are each induced; only the first four create a new configuration generation. For each of the three ordered calls, `BUSY`, returned `TIMEOUT`, and client timeout produce at most four 100 ms attempts separated by exactly 100, 200, then 400 ms backoff; all other results are attempted once. Every future is tagged with configuration generation and Agent session ID: delayed responses after timeout or session change are ignored and cannot open the motion gate. Permanent failure or retry exhaustion reports the cause and leaves the gate false. Across ten repetitions of each new-session trigger, non-default motor model, PWM offsets, and battery threshold are restored in that order after MCU reset and idempotently reapplied after Agent-only recovery; ten normal uptime wraps trigger no generation. No bus persistence or other command is changed or replayed. |
| `VER-HOST-COMMISSION-001` | Native core and ROS graph tests prove the guarded C++ utility accepts only a 0.01--0.25 RPS target and exactly one `/mentor_pi/configuration/motion_authorization` publisher whose identity is `/mentor_pi/configuration_supervisor`; zero, duplicate, wrongly named, disappearing, or changed generation/session authorization fails closed. A second `/mentor_pi/motors/command` publisher also prevents motion. A drive publish gap greater than 100 ms initiates stop and never resumes the drive phase. Heartbeat sequence/uptime or diagnostic uptime regression, any selected lease-expiry or watchdog-counter change, and a nonzero watchdog mask abort. Selected speed above 0.50 RPS, wrong measured or encoder direction, unselected speed above 0.02 RPS, and unselected displacement of at least two ticks each abort. Admission and target echo without a correctly directed response of at least `max(0.002 RPS, 10% of target)` and two correctly directed encoder ticks cannot pass. Post-stop success requires the latest state to report all four targets zero and all four `abs(measured_rps) < 0.002 RPS`; a later target invalidates an earlier zero-target latch, and residual speed with zero targets fails. `SIGINT`/`SIGTERM` exercise the bounded zero phase, while injected stop-control and exception/shutdown paths request an immediate all-motor zero; loss of ROS publication leaves the independent 200 ms MCU lease as the documented fallback. |
...
| `VER-UNIT-MOTOR-GATE-001` | For SAFE-006 and SAFE-007, the default motor configuration accepts every valid selected all-zero subset as a stop without arming, rejects a selected nonzero or mixed zero/nonzero message atomically as `UNSUPPORTED`, applies no output, and refreshes no motion lease. The commissioning configuration accepts ±0.25 RPS, rejects adjacent finite values ±0.251 as `OUT_OF_RANGE`, and clamps no target; control output never exceeds ±300 permille. Portable tests assert that the evidence-derived model factor is currently `-1` for JGA27 and `+1` for the other retained models while explicitly marking those assertions provisional rather than HIL evidence. |
| `VER-UNIT-MBOX-001` | Motor, PWM, LED, RGB, and OLED tests first publish a complete state and then update one selected element in the same session; every selected element changes and every unselected element remains unchanged. Multiple writes to the same selected element before one read expose only the newest generation and increment overwrite accounting exactly as the ROS contract specifies. For each merged mailbox, leave a selected field unread, reset its session ownership, then publish a disjoint field: the old field-valid generation is zero, its pending value is absent, the new field is present, and the monotonic mailbox generation does not restart. The triple-buffer test races producer `Publish` plus producer `DiscardLatest` against `ConsumeLatest`; every consumed object remains whole and strictly newer, a discard winning after the consumer's initial unread observation returns no value, and the final non-discarded generation remains observable. Controller tests repeat the reset at a real session transition: unread gen-1 motor/PWM/LED/RGB/OLED fields never arm, target, or render when gen 2 first selects a different field; stale whole-command bus/buzzer work performs no hardware transaction. A separate contrast first applies LED/RGB/OLED gen-1 state, then proves a disjoint gen-2 update preserves that worker-owned unselected state. Bus motion and buzzer complete-state slots otherwise expose only the newest same-session complete write. All storage remains at its documented depth. |
...
|---|---|---|
| `VER-HIL-LED-001` | `ROS-LED-01`, `HW-LED-01` | Each of three IDs independently performs steady off/on, one finite cycle, multiple finite cycles, and indefinite repetition followed by replacement. Measured on/off intervals are within ±2% or 2 ms, whichever is larger; invalid IDs have no effect. LED3 is never changed merely by transport activity. |
...
| `VER-HIL-MOT-001` | `ROS-MOT-01`, `HW-MOT-01` | The test is staged. First, flash the normal locked image, prove a selected nonzero command is rejected while zero/stop succeeds, keep all bridge outputs at zero, manually rotate each raised wheel in the declared positive direction, and record raw counter and normalized `motors/state` signs; JGA27's provisional `-1` factor is explicitly confirmed or corrected before powered work. Second, use only the dual-acknowledged commissioning image, raised-wheel/equivalent guarding, and a current-limited supply; exercise one channel at a time at magnitudes no greater than 0.25 RPS, prove output never exceeds 300 permille, verify stop/lease/session kill paths, and stop on wrong sign, unexpected current, oscillation, or saturation. These stages characterize initial polarity and PID behavior but do not by themselves release full-range production motion. To close D3 for nonzero production control, the reviewed candidate and recorded HIL must additionally qualify or replace every channel/model output sign, encoder sign/constant, PID/filter/deadband profile, full documented RPS range, subset merge, zero behavior, model-change behavior, and independent state. An actual model change is `BUSY` while any target is nonzero and resets all PID state on success; requesting the already active model while a target is nonzero returns `OK` without a reset. Callback-acceptance-to-`MotorControlTask` mailbox-consumption p99 is at most 20 ms, the separate PID/output loop remains 100 Hz, and no output exceeds the authority of the exact image under test. |
...
| `VER-HIL-RGB-001` | `HW-RGB-01` | Each `update_mask` value 1, 2, and 3 changes only selected pixels. Off, primary, secondary, and white component values are exact; the serialized waveform meets the device timing; zero/unknown masks are rejected. |
...
LED commands at 1 Hz, one finite buzzer pattern every 10 seconds, RGB commands
at 10 Hz, and OLED commands at 0.2 Hz. Exercise all six services sequentially
...
Before full motor release qualification, this run shall use the acknowledged
commissioning image on the guarded/current-limited fixture, keep every target
within ±0.25 RPS, and prove the ±300 permille output cap. A normal locked-image
...
pattern per minute, RGB at 30 Hz, OLED at 1 Hz, and all nominal
state/heartbeat/diagnostic rates. Exercise all six services once per minute
...
guarded commissioning image until a later exact release candidate has completed
full PID/polarity qualification; otherwise the soak uses zero/stop motor
traffic in the normal locked image.
...
| `VER-RECONNECT-USB-001` | During sequence-coded 500 Hz traffic, physically disconnect/reconnect USB 100 times, rotating 1 s, 2 s, and 5 s outages with at least 10 s connected between cycles. Each cycle stops motors within 200 ms, holds PWM/bus state, replays no old generation, resets no MCU, restores all 20 endpoints within 5 s of device and Agent availability, and moves only after a fresh command. |
...
| `VER-FAULT-UART-001` | Independently inject USART1 framing, noise, parity, and hardware-overrun faults before the motor pre-check, between that check and the arm authority write, between duty-channel writes, between the final duty write and post-check, and immediately after the post-check while the controller critical section remains held. The exact error counter increments. A pre-existing latch stops and rejects without a write; a latch during arm or the four-channel update is caught by the post-check and every output is stopped before `BUSY` returns; after the post-check, no write remains and the deferred ISR stop wins at critical-section exit. Motors disarm by the next 1 kHz release, task-context teardown/reset occurs, the priority-4 top half performs no application work, the inhibit has no host unlock, and a clean session recovers within 5 s after fault removal through the normal transport-open path. |
...
| Middleware/storage bounds | Entity counts are exactly one node, seven publishers, seven subscriptions, six services, zero clients, and history depth eight. Mailbox, event FIFO, service-slot, message, sequence, and string backing capacities equal the architecture/ROS contract and never grow. |
...
Copy the [qualification evidence ledger](qualification-evidence-ledger.md) into
the immutable candidate evidence root and complete one row per verification ID
with revision, environment, start/end time, result, evidence links, and measured
margin. The report includes the completed requirement and audit trace matrices,
all resource arithmetic, and a list of nonblocking should-level deviations. A
failed endurance/recovery case shall be rerun in full; rerunning only the
failing interval is not acceptance. An automated campaign execution pass with
unobserved physical metrics remains incomplete and cannot close a row by
itself.
